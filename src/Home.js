...
import { View ,FlatList,StyleSheet, Text,SafeAreaView,StatusBar,Alert,
...
const Home = ({ navigation }) => {
  ...
  (async () => {
    try {
        ...
        newdata = await json.menu.map((item, index) => {return ({'id': index, ...item})});
    } catch (e) {
  ...
  if (isloading || loaddata) {
    ...
    const Item = ({ title, description,price,imageurl }) => {
      ...
      return (<View style={styles.item}>
        ...
        resizeMode="contain" source={{uri: `https://github.com/Meta-Mobile-Developer-PC/Working-With-Data-API/blob/main/images/${correction}?raw=true`}} Alt="Alternative text" />
      </View>);
    };
...
source={require("./img/Hero.png")}/>
      ...
      <SearchBar
        ...
        round={true}
        iconColor="grey"
        containerStyle={{elevation: 0, borderWidth: 0, backgroundColor:"#495E57", alignSelf: "center" ,width: "80%"}}
        inputContainerStyle={{
...
<Filter text="Desserts" index={3} />
      ...
      <FlatList
        data={data}
        renderItem={({item}) => <Item title={item.name} price={item.price} description={item.description} imageurl={item.image} />}
